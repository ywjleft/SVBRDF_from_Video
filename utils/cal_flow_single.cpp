...
extern "C" void calculate_flow(float* camera_params, double fovRadian, int frameA, int frameB, float* flow_array) {
    int res = 256;
    ...
    lookAt(camera_params + frameA * 9, extrinsic_A);
    /*for (int i = 0; i < 16; ++i) {
        std::cout << extrinsic_A[i] << " ";
    }
    std::cout << std::endl;*/
    float extrinsic_B[16];
    lookAt(camera_params + frameB * 9, extrinsic_B);
    for (int i = 0; i < res; ++i) {
        for (int j = 0; j < res; ++j) {
            ...
            std::cout << std::endl;*/
            flow_array[i * res * 2 + j * 2] = j - uv[0] / uv[2];
            flow_array[i * res * 2 + j * 2 + 1] = i - uv[1] / uv[2];
        }
    }
}
