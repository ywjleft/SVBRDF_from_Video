...
extern "C" void calculate_flow(float* camera_params, double fovRadian, float* flow_array) {
    int res = 256;
    ...
    for (int k = 0; k < 140; ++k) {
        ...
        lookAt(camera_params + k * 9, extrinsic_A);
        /*for (int i = 0; i < 16; ++i) {
            std::cout << extrinsic_A[i] << " ";
        }
        std::cout << std::endl;*/
        float extrinsic_B[16];
        lookAt(camera_params + (k + 1) * 9, extrinsic_B);
        for (int i = 0; i < res; ++i) {
            for (int j = 0; j < res; ++j) {
