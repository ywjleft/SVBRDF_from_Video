...
class Model:
    ...
    def __init__(self, input_images, input_flows, input_feature, input_feature_secondary, dyn_batch_size, useSecondary=True, ngf=64, generatorOutputChannels=64, reuse_bool=False,  pooling_type="max", last_convolutions_channels=[64,64,48,32,16,9], useCoordConv = True, firstAsGuide = False, NoMaxPooling = False, useCPU = False):
    ...
    def create_model(self):
        with tf.variable_scope("trainableModel", reuse=self.reuse_bool) as scope:
            generator_output, self.generator_secondary_output = self.__create_generator(self.input_images, self.generatorOutputChannels, self.reuse_bool)
            self.generator_output = warp_const(generator_output, self.input_flows)
